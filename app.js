...
db.serialize(() => {
  ...
  db.run(
    "create table if not exists members(id integer primary key,ymd,member,name,yen,kosuu)"
  );
...
function func1(req, res) {
  ...
  db.serialize(() => {
    ...
    db.run(
      "insert into members(ymd,member,name,yen,kosuu) values(?,?,?,?,?)",
      getDate,
      getMember,
      getGoods,
      getYen,
    ...
    db.each("select * from members", (err, row) => {
      console.log(
        `${row.ymd} ${row.member} ${row.name} ${row.yen} ${row.kosuu}`
      );
    });
    db.all("select * from members", (err, rows) => {
